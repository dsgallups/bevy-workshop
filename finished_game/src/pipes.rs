...
use crate::{CANVAS_SIZE, walls::WALL_Y_LEN};

const PIPE_X_LEN: f32 = 50.;
const PIPE_GAP: f32 = 80.;

pub fn plugin(app: &mut App) {
    app.add_systems(Startup, spawn_first_pipe_pair)
        .add_observer(insert_new_pipe);
}
...
fn spawn_first_pipe_pair(mut commands: Commands) {
...
fn insert_new_pipe(trigger: Trigger<InsertPipe>, mut commands: Commands) {
    ...
    */
    ...
    commands.spawn((
        ...
        Transform::from_xyz(pair_x, top_pipe_center_y, 2.),
    ));
    ...
    commands.spawn((
        ...
        Transform::from_xyz(pair_x, bottom_pipe_center_y, 2.),
    ));
